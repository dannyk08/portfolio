...
export default function Contact() {
  ...
  const handleOnSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    const newErrors = Object.entries(inputs).reduce(
      (acc, [key, value]) => ({
        ...acc,
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Contact Daniel (Danny) Joffe Web Developer for your web projects."
        />
      </Head>
      <Page>
        ...
        <div className={styles.cardContainer}>
          <Card>
            <form onSubmit={handleOnSubmit}>
              ...
              <Primary>{formSending ? "Sending..." : "Send"}</Primary>
            </form>
            {showFormErrorMessage && (
              <FormSubmitMessage success={false}>
                Sorry about that! There was an error sending your message.
                Please reach out on LinkedIn!
              </FormSubmitMessage>
            )}
            {showFormSuccessMessage && (
              <FormSubmitMessage success={true}>
                Success! Your message was delivered successfully, please allow a
                couple days to let me get back to you!
              </FormSubmitMessage>
            )}
          </Card>
        </div>
